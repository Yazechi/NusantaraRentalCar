...
// Build query
$where_clause = "1=1";
if (!empty($status_filter) && in_array($status_filter, ['pending', 'approved', 'cancelled', 'completed'])) {
    $where_clause = "o.status = '$status_filter'";
}

// Get total orders
$total_result = $conn->query("SELECT COUNT(*) as count FROM orders o WHERE $where_clause");
...
";
$orders = $conn->query($orders_query)->fetch_all(MYSQLI_ASSOC);

// Get status stats
$stats = [];
foreach (['pending', 'approved', 'cancelled', 'completed'] as $st) {
    $result = $conn->query("SELECT COUNT(*) as count FROM orders WHERE status = '$st'");
    $stats[$st] = $result->fetch_assoc()['count'];
}
?>
